...
│   └── app.js                 # HUD interactivity (audio visualizer, SSE scenarios, TTS)
└── tests/
    ├── __init__.py
...
| `on_user_started_speaking()` | VAD-triggered handler: increments fence, cancels pending tools, flushes TTS audio |
| `_measure_cutoff_latency()` | Computes elapsed time from audio onset to VAD trip; applies bounds (35–145 ms simulation) |
...
| `get_med_dosage()` | `@function_tool`-decorated: dispatches `MedDoseTool`, then verifies fence identity before yielding results |

**Interruption Flow (`on_user_started_speaking`)**

```python
async def on_user_started_speaking(self) -> None:
    # 1. Measure cutoff latency (VAD trip timing)
    ...
    # 2. Increment atomic fence token
    self.fence_state.request_id += 1

    # 3. Cancel in-flight tool lookups
    cancelled = await self._cancel_pending_tool_calls()

    # 4. Flush Rime TTS buffer
    await self._stop_tts_audio()
```

**State Fencing (`get_med_dosage`)**

```python
@function_tool(name="get_med_dosage", description=...)
async def get_med_dosage(self, medication, dose_mg, patient_weight_kg):
    fence_id = self.fence_state.request_id  # capture at dispatch time
    result = await self.med_tool(medication=..., fence_id=fence_id)
    # Critical: only accept result if fence is still current
    if self.fence_state.request_id != fence_id:
        print("[STATE FENCE VIOLATION PREVENTED] Discarded stale tool output")
        return None
    return result
```
...
| Medication | Key Indications in Database |
|------------|---------------------------|
...
- `speech_provider`: provider name, model_id (`coda`), speaker_id (`lawton`), language (`eng`), audio format (`PCM 22.05 kHz Mono Streaming`), transport type
...
- Uses `coda` model with `lawton` speaker
- Falls back to OpenAI TTS (`tts-1`, `alloy` voice) if Rime key is absent
- Returns audio as `audio/mp3` stream
...
**Design tokens:**
- Background: `#080b12` (deep space navy)
- Accent: `--accent-cyan: #00f0b5` (teal)
- Status: green (online), amber (warning), red (error/interrupt)
- Fonts: Inter (sans), Outfit (headings), JetBrains Mono (technical/mono)
...
| Server Event | HUD Action |
|-------------|------------|
| `VAD_INTERRUPT` | Sets `isAudioPlaying = false`, updates cutoff display, increments fence ID |
| `STATE_FENCE_DISCARD` | Appends log to terminal with `tag-fence` styling |
| `RIME_TTS_STREAM` | Sets `isAudioPlaying = true`, updates visualizer, appends log |
| `TOOL_SUCCESS` | Appends verified dosage to terminal with `tag-tool` styling |
| `SCENARIO_COMPLETE` | Appends summary, closes SSE connection |
...
- Responsive grid layout: 3-column on desktop, stacked on mobile (max-width: 1200px)
...
|-------|---------|--------|------------|
| Idle (`is_speaking=False`) | User speech detected (non-interrupt) | Begin new LLM turn, set `request_id = N` | Tool Dispatch |
| Tool Dispatch | Tool invoked | Tag with `fence_id = request_id`, track in `_pending_calls` | Awaiting Result |
| Awaiting Result | VAD `user_started_speaking` | Increment `request_id`, cancel task, flush TTS | Idle |
...
| Metric | Target SLA | Uncached (Cold) | Cached (Warm) | Measurement Method |
|--------|------------|-----------------|---------------|--------------------|
| **Audio Cutoff (VAD → silence)** | **< 150 ms** | **0.08 ms** | **0.04 ms** | Real `time.perf_counter()` wall-clock |
| STT First Token (Deepgram) | < 300 ms | 350 ms | 185 ms | WebSocket streaming frame |
| LLM TTFT (GPT-4o-mini) | < 300 ms | 410 ms | 140 ms | Streaming token completion |
| Rime TTFA (First Audio Byte) | < 500 ms | 2425 ms (Cold TLS) | 470 ms (Stream chunk) | Live Rime production API |
| Formulary DB Lookup | < 3000 ms | 2500 ms (Simulated) | 0.05 ms (In-Memory) | Async cancellable lookup |
| Round-Trip (End-to-End) | < 800 ms | 3100 ms | 650 ms | End-to-end conversation flow |
...
tests/test_interruption.py::TestInterruptionLatency::test_agent_get_med_dosage_state_fencing   PASSED
...
| Test | Description |
|------|-------------|
| `test_cutoff_latency_under_150ms` | Creates a 2-second `MedDoseTool` task, interrupts after 1 second via `task.cancel()`, and asserts cancellation < 150 ms |
| `test_stale_tool_output_discarded` | Dispatches tool with `fence_id=10`, bumps to `fence_id=11`, and verifies the stale result is discarded |
| `test_newest_request_id_accepted` | Confirms results matching the active `fence_id` are accepted and contain `status="APPROVED"` |
| `test_agent_get_med_dosage_state_fencing` | Full `FieldMedicAgent` integration: dispatches `get_med_dosage`, triggers `on_user_started_speaking()` mid-call, and asserts the tool returns `None` |
| `test_multiple_rapid_interruptions_stress` | 5 consecutive `get_med_dosage` calls with fence bumps between each; verifies all 5 fence IDs increment correctly |
| `test_tts_provider_factory` | Validates `create_tts_engine()` returns Rime config when key present, and mock fallback when absent |
...
============================================================
[VAD INTERRUPT] User speech detected during agent turn!
[LATENCY] Audio Cutoff Latency: 35.00 ms (< 150ms target)
[STATE FENCE] Incrementing Fence ID from 1 -> 2
============================================================
[STATE FENCE] Cancelled 1 in-flight tool execution(s).
[STATE FENCE] Active lookup for 'morphine' (Fence ID: 1) was CANCELLED by user interruption.
...
[RIME TTS AUDIO OUTPUT]: 'Verified: Pediatric dose for Fentanyl on 25 kg patient is 25 to 50 micrograms IV.'
  ...
  - Average Cutoff Latency: 35.00 ms
===========================================================
```

### SSE Scenario Demo (Web HUD)
